...
{
    public class MaxBannerAds : IDisposable
    {
        ...
        public MaxBannerAds(string bannerUnitId)
        {
            ...
            MaxSdk.CreateBanner(bannerUnitId, adViewConfiguration);
        ...
        public void ToggleBannerAds(bool shouldShowAds)
        {
            if (shouldShowAds)
            {
                Debug.Log($"[{MaxBannerLogTag}] Show banner ads: {this.bannerUnitId}");
                MaxSdk.ShowBanner(this.bannerUnitId);
            }
            else
            {
                Debug.Log($"[{MaxBannerLogTag}] Hide banner ads: {this.bannerUnitId}");
                MaxSdk.HideBanner(this.bannerUnitId);
            }
        }

        public void DestroyBannerAds()
        {
            Debug.Log($"[{MaxBannerLogTag}] Destroy banner ads: {this.bannerUnitId}");
            MaxSdk.DestroyBanner(this.bannerUnitId);
        }
        ...
        {
            Debug.Log($"[{MaxBannerLogTag}] Banner ad load failed with id {adUnitId} with error code: {errorInfo.Code}, information: {errorInfo.Message}");
        }
        ...
        {
            Debug.Log($"[{MaxBannerLogTag}] Banner ad revenue paid with id {adUnitId} at placement {adInfo.Placement}. Revenue: {adInfo.RevenuePrecision}");
        }
        ...
        {
            Debug.Log($"[{MaxBannerLogTag}] Ad review ad with unit id {adUnitId} creative id generated: {adReviewCreativeId}");
        }
